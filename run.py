...
from db_utlity import SQlAlchemyOperation
import constants
...
class OutputOperation(Operation):
    ...
    def find_300(self,url):
        txt=str(requests.get(url).status_code)
        return txt

    def get_redirected_url(self,df):
        #todo
        pattern=r'^3+'
        txt=df[df['status']=='300']
        if re.findall(pattern,txt) !=[]:
            df['redirected_to_url']=(txt)
    
    def set_data(self,df_input):
        for i in range(0,50):#len(df)):
            try:
                df_input.iloc[i,-1]=self.find_300(df_input.iloc[i,-2])
            except ConnectionError as ce:
                print(ce)
    def temp_operation(self,filepath):
        i_o_object=InputOperation()
        dfi_o_object.get_data(filepath)
